...
1. Download PDFs via institutional access
...
- Reference critical perspectives (Badhwar, Mehaffey, Kaul, Miller, Chikwe)
- Tone: expert skepticism — enthusiastic about real advances but always questioning

## HARDWIRED EDITORIAL PRIVACY RULES (non-negotiable)

The publication is authored under a pen name. These rules are non-negotiable and
apply to every file, prompt, commit message, and output generated by this project:

1. **Byline is always `E. Nolan Beckett, MD`.** Never use the editor's real name in
   any public-facing surface (website, podcast, outreach copy, commits to origin/main,
   social posts, README). The real name never appears in any content generated by
   this pipeline or any prompt sent to Claude/OpenAI.

2. **Never mention "[institution]," "[institution]," the editor's employer, or imply an
   institutional affiliation.** This includes:
   - No `[institution].edu` or employer email addresses anywhere
   - No references to the editor's institution in docs, code comments, commit
     messages, PR descriptions, or AI output
   - No "institutional partnership" language that identifies a specific institution
   - Cited research may reference author institutions (academic citation is neutral),
     but outputs must never describe the *editor* as affiliated with anywhere
   - The `tools/institution_auth.py` / EZproxy workflow (Tier 4 post-launch) is
     named generically; do not introduce institution-specific names

3. **If in doubt, omit.** It is always safer to generalize ("the editor",
   "an academic medical center", "institutional access") than to risk outing the
   pen name. The reveal is a future strategic decision controlled by the editor —
   never pre-empt it in code or content.

4. **This rule overrides everything else.** If a prompt, template, or user input
   contains the editor's real name or institution, scrub it before processing.
   Flag the attempted leak to the user immediately.
